...
5. **Si aún no hay suficiente holdout collector** (p.ej. &lt; N sesiones etiquetadas; N default ~500 intent / umbral documentado en Status): el cron **entrena candidato** y puede dejarlo en **shadow**, pero **no publica** (gate = skip / fail con razón `insufficient_full_schema_holdout`). UI sigue reglas/YAML.
