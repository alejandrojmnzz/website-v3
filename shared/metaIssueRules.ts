/**
 * Shared meta issue rules for client preview and documentation.
 * Server `meta` validator remains authoritative in the validation store;
 * this module keeps client keystroke checks aligned on codes/messages.
 */

import { isUsableOgImageUrl } from "./ogImageUrl";
...
export const VALID_ROBOTS = ["index", "noindex", "follow", "nofollow", "none", "all"];

export interface SharedMetaIssue {
  code: string;
  message: string;
}
...
/** Client/editor preview — same codes as meta validator where overlapping. */
export function collectSharedMetaIssues(
  meta: Record<string, unknown> | null | undefined,
): SharedMetaIssue[] {
  const issues: SharedMetaIssue[] = [];
  const m = meta || {};
  ...
  if (
    changeFrequency &&
    !VALID_CHANGE_FREQUENCIES.includes(
      changeFrequency as (typeof VALID_CHANGE_FREQUENCIES)[number],
    )
  ) {
    issues.push({
