...
The migration script (`scripts/admin/migrate-gcs-multisite.ts`) uses raw `@google-cloud/storage` clients (source + target) to bypass the `gcs` singleton and its write-block. It migrates flat `media/` → `{site}/media/`, unifies sync prefixes (`sync/{site}/` → `{site}/sync/`), and moves platform user state to `multisite-user-store/`. Registry URL rewrites parse JSON structurally and write back with `JSON.stringify(..., null, 2) + "\n"`.
