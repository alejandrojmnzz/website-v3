...
- **Circular trap (meta vs body):** Live saves validate SEO meta **and** editor.required fields on the post-write merged document. If both `meta.description` and body `description` are empty strings, fixing only one side still fails the other gate. Remedy: set all missing paths in **one** multi-field write — MCP `batch_update_fields` (or `edit-sections` with multiple `update_field` ops). `update_meta_fields` alone cannot set body `description`; `update_section_fields` alone cannot set `meta.description`. Failures return `code: live_required_fields` + `missing_fields` and MCP `action_required: fix_live_required_fields`.
