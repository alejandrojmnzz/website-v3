...
const ECOMMERCE_EVENT_CATALOG = [
  ...
  },
  {
    name: "add_to_cart",
    wired: true,
    description: "CTA with tracking=add_to_cart (e.g. payment-component configurator)",
  ...
  app.get("/api/ecommerce/events", (_req, res) => {
    try {
      res.json({
        ...
        education: {
          summary:
            "Ecommerce funnel events are purchasable-gated. Forms/conversions are separate. CMS does not manage billing plans. Purchase completes off-site.",
          advanced_paths: [
