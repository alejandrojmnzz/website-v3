...
function writeTopLevelFieldsToPerEntryFile(opts: {
  ...
  try {
    const perEntryDir = path.join(process.cwd(), process.env.CONTENT_FOLDER || "content", getFolder(contentType), slug);
...
export async function renameContentSlug(
  ...
  const folderPath = path.join(process.cwd(), process.env.CONTENT_FOLDER || "content", contentFolder, resolvedFolderSlug);
...
export async function deleteContentEntry(
  ...
  const folderPath = path.join(process.cwd(), process.env.CONTENT_FOLDER || "content", typeFolder, resolvedSlug);
  ...
  const allowedBase = fs.realpathSync(path.join(process.cwd(), process.env.CONTENT_FOLDER || "content", typeFolder));
...
export async function createContentEntry(
  ...
  const folderPath = path.join(process.cwd(), process.env.CONTENT_FOLDER || "content", getFolder(type), folderSlug);
  if (fs.existsSync(folderPath)) {
    return { success: false, statusCode: 409, error: `A ${type} with slug "${folderSlug}" already exists` };
  }
