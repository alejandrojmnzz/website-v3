...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={cn(
      // Above Dialog (z-[10000]): nested confirms (Fields save → variant layer) must stay clickable.
      "fixed inset-0 z-[10001] bg-black/80 pointer-events-none data-[state=open]:pointer-events-auto data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 duration-200",
      className
    )}
    {...props}
    ref={ref}
...
const AlertDialogFooter = ({
  ...
  <div
    className={cn(
      "flex min-w-0 flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2",
      className
    )}
    {...props}
  />
